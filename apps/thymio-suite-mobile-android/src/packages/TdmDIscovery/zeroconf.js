...
export default class Zeroconf extends EventEmitter {
  ...
  addDeviceListeners() {
    if (Object.keys(this._dListeners).length) {
      return this.emit(
        'error',
        new Error('RNZeroconf listeners already in place.'),
      );
    }

    this._dListeners.start = DeviceEventEmitter.addListener(
      'RNZeroconfStart',
      () => this.emit('start'),
    );

    this._dListeners.stop = DeviceEventEmitter.addListener(
      'RNZeroconfStop',
      () => this.emit('stop'),
    );

    this._dListeners.error = DeviceEventEmitter.addListener(
      'RNZeroconfError',
      err => {
    ...
    this._dListeners.found = DeviceEventEmitter.addListener(
      ...
      service => {
        ...
        // console.log('ZEROCONF:::found:::', JSON.stringify(service));
    ...
    this._dListeners.remove = DeviceEventEmitter.addListener(
      ...
      service => {
        ...
        // console.log('ZEROCONF:::remove:::', service);
    ...
    this._dListeners.resolved = DeviceEventEmitter.addListener(
      ...
      service => {
        if (!service || !service.name) {
          console.log('ZEROCONF:::resolved:::ERROR:::', service);
          this.emit('update');
          return;
        }

        // console.log('ZEROCONF:::resolved:::', JSON.stringify(service));
  ...
  removeDeviceListeners() {
    Object.keys(this._dListeners).forEach(name =>
      this._dListeners[name].remove(),
    );
    this._dListeners = {};
  }
  ...
  getListeners = () => {
    return Object.keys(this._dListeners).map(name => name);
  };
  ...
  scan(
    ...
    if (Platform.OS === 'android') {
      RNZeroconf.scan(type, protocol, domain, 'NSD');
    } else {
  ...
  stop(implType = ImplType.NSD) {
    if (Platform.OS === 'android') {
      // console.log('ZEROCONF:::stop:::', implType);
